...
fn listSimilarity(listA: []const u32, listB: []const u32) u32 {
    var sum: u32 = 0;
    for (listA) |a| {
        var count: u32 = 0;
        for (listB) |b| {
...
test {
    try std.testing.expectEqual(31, listSimilarity(&.{ 3, 4, 2, 1, 3, 3 }, &.{ 4, 3, 5, 3, 9, 3 }));
}
...
pub fn main() !void {
    ...
    var listA = std.ArrayList(u32).init(alloc);
    defer listA.deinit();
    var listB = std.ArrayList(u32).init(alloc);
    defer listB.deinit();

    while (try iter.next()) |line| {
        var seq = std.mem.splitSequence(u8, line, "   ");
        try listA.append(try std.fmt.parseInt(u32, seq.next().?, 10));
        try listB.append(try std.fmt.parseInt(u32, seq.next().?, 10));
    }

    const sum = listSimilarity(listA.items, listB.items);
