...
fn isSafe(lvls: []const i32) bool {
    ...
    for (lvls[1..]) |lvl| {
        ...
        const d: Dir = if (diff > 0) .inc else .dec;
        if (dir != null and d != dir) return false;
        prev = lvl;
        dir = d;
    }
    return true;
...
test {
    try std.testing.expectEqual(true, isSafe(&.{ 7, 6, 4, 2, 1 }));
    try std.testing.expectEqual(false, isSafe(&.{ 1, 2, 7, 8, 9 }));
    try std.testing.expectEqual(false, isSafe(&.{ 9, 7, 6, 2, 1 }));
    try std.testing.expectEqual(false, isSafe(&.{ 1, 3, 2, 4, 5 }));
    try std.testing.expectEqual(false, isSafe(&.{ 8, 6, 4, 4, 1 }));
    try std.testing.expectEqual(true, isSafe(&.{ 1, 3, 6, 7, 9 }));
}
...
pub fn main() !void {
    ...
    while (try iter.next()) |line| {
        ...
        safe += if (isSafe(lvls.items)) 1 else 0;
        try lvls.resize(0);
    }
