...
//     the right list is 3. The distance between them is 2. The second-smallest
//     number in the left list is 2, and the second-smallest number in the
//     right list is another 3. The distance between them is 1. The
//     third-smallest number in both lists is 3, so the distance between them
//     is 0. The next numbers to pair up are 3 and 4, a distance of 1. The
//     fifth-smallest numbers in each list are 3 and 5, a distance of 2.
...
const ListDistError = error{LengthMismatch};

fn listDist(listA: []i32, listB: []i32) ListDistError!u32 {
    if (listA.len != listB.len) return ListDistError.LengthMismatch;
    std.mem.sort(i32, listA, {}, comptime std.sort.asc(i32));
    std.mem.sort(i32, listB, {}, comptime std.sort.asc(i32));
    var sum: u32 = 0;
    for (listA, 0..) |_, i| {
...
test {
    var listA = [_]i32{ 3, 4, 2, 1, 3, 3 };
    var listB = [_]i32{ 4, 3, 5, 3, 9, 3 };
    const sum = try listDist(&listA, &listB);
    try std.testing.expectEqual(11, sum);
}
...
pub fn main() !void {
    ...
    defer listA.deinit();
    var listB = std.ArrayList(i32).init(alloc);
    defer listB.deinit();
    ...
    while (try iter.next()) |line| {
        ...
        const a = seq.next();
        const b = seq.next();
        try listA.append(try std.fmt.parseInt(i32, a.?, 10));
        try listB.append(try std.fmt.parseInt(i32, b.?, 10));
    }

    const sum = try listDist(listA.items, listB.items);
